...
class _AutomationScreenState extends State<AutomationScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final readyChecks = checks.where((row) {
      final check = asMap(row);
      return check['ok'] == true || check['ready'] == true;
    }).length;
    final readinessPercent = checks.isEmpty
        ? (ready ? 100.0 : 0.0)
            ...
            child: Column(
              ...
              children: [
                CompactPageHeader(
                  title: 'Autopilot',
                  status: _enabled
                      ? '${_mode == 'manage' ? 'Manage' : 'Observe'} · ${readinessPercent.round()}% ready'
                      : 'Disabled',
                  statusColor: !_enabled
                      ? ByteSqueezeColors.muted
                      : (ready
                            ? ByteSqueezeColors.mint
                            : ByteSqueezeColors.amber),
                  summary:
                      '${autopilot['eligible'] ?? 0} eligible · ${autopilot['selected'] ?? 0} selected · ${continuousLearning['pending_count'] ?? continuousLearning['pending'] ?? 0} reviews',
                ),
                SurfaceCard(
                  borderColor:
                      (_enabled && ready
                    ...
                    children: [
                      ...
                      if (!ready && checks.isNotEmpty) ...[
                        ...
                        Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                '${asMap(checks.firstWhere((row) {
                                      final check = asMap(row);
                                      return check['ok'] != true && check['ready'] != true;
                                    }, orElse: () => <String, dynamic>{}))['label'] ?? 'Setup needs attention'}',
                                maxLines: 1,
                      ...
                      FilledButton.icon(
                        onPressed:
                            ...
                            ? () => _run(
                                widget.controller.runAutopilot,
                ...
                const SectionHeader(title: 'Latest decisions'),
                if (decisions.isEmpty)
                  const _CompactNotice(
                    icon: Icons.route_outlined,
                    message: 'Run a decision cycle to see what Autopilot will do next.',
                  )
                else
                  ...decisions
                      .take(4)
                ...
                SurfaceCard(
                  ...
                  child: ExpansionTile(
                    ...
                    subtitle: const Text(
                      'Profiles, previews, and after-watch feedback',
                    ),
                ...
                SurfaceCard(
                  ...
                  child: ExpansionTile(
                    ...
                    subtitle: Text(
                      '${_mode == 'manage' ? 'Manage' : 'Observe'} · ${_minSavings.round()}% minimum savings · ${_batchLimit.round()} per cycle',
                    ),
                        ...
                        children: [
                          ...
                          SwitchListTile.adaptive(
                            ...
                            onChanged: widget.controller.canControl
                                ...
                                : null,
                            title: const Text(
                              'Continue learning after playback',
                            ),
                            subtitle: const Text(
                ...
                SurfaceCard(
                  ...
                  child: ExpansionTile(
                    ...
                    subtitle: Text(
                      ready ? 'All checks passed' : 'Needs attention',
                    ),
                ...
                SurfaceCard(
                  ...
                  child: ExpansionTile(
                    ...
                    subtitle: Text(
                      '${learning['feedback_count'] ?? 0} learned decisions',
                    ),
                    childrenPadding: const EdgeInsets.fromLTRB(12, 0, 12, 12),
                    children: [
                      _SmartPresetCard(
                        ...
                        learning: learning,
                        sceneAi: asMap(
                          widget.controller.smartPresets['scene_ai'],
                        ),
                      ),
                    ],
                  ),
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Automation policy saved.')));
    } catch (error) {
      if (!mounted) return;
      ...
      SnackBar(
        content: Text(
          '${profile == 'safe'
              ...
              : profile == 'hands_off'
              ? 'Hands-off'
              : 'Balanced'} loaded. Review it, then save.',
        ),
      ),
    );
  }
    ...
    return FractionallySizedBox(
      ...
      child: ListView(
        ...
        children: [
          ...
          const Text(
            'The complete phone workflow, from setup to feedback.',
            style: TextStyle(color: ByteSqueezeColors.muted),
          ),
          const SizedBox(height: 18),
          ...sections.map(
            ...
            (step) => Padding(
              ...
              child: Row(
                ...
                children: [
                  CircleAvatar(
                    radius: 14,
                    backgroundColor: ByteSqueezeColors.cyan.withValues(
                      alpha: .16,
                    ),
                    child: Text(
                      step.$1,
...
class _AutopilotTrainingCard extends StatefulWidget {
  ...
  @override
  State<_AutopilotTrainingCard> createState() => _AutopilotTrainingCardState();
}
      ...
      builder: (context) => SafeArea(
        child: Column(
          ...
          children: [
            const ListTile(
              title: Text(
                'What should ByteSqueeze improve?',
                style: TextStyle(fontWeight: FontWeight.w800),
              ),
...
class _CompactNotice extends StatelessWidget {
  const _CompactNotice({required this.icon, required this.message});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Icon(icon, color: ByteSqueezeColors.muted, size: 19),
          const SizedBox(width: 9),
          Expanded(
            child: Text(
              message,
...
class _SmartPresetCardState extends State<_SmartPresetCard> {
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(
                ready ? Icons.psychology_alt_rounded : Icons.school_outlined,
                color: ready ? ByteSqueezeColors.mint : ByteSqueezeColors.cyan,
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      ready ? 'Automatic selection ready' : 'Still learning',
          ...
          if (_episodeAiEnabled) ...[
            ...
            DropdownButtonFormField<int>(
              ...
              items: const [3, 4, 5, 6, 8]
                  ...
                  .toList(),
              onChanged: widget.controller.canControl
                  ? (value) => setState(() => _episodeAiFrameCount = value ?? 4)
                  : null,
            ),
            const SizedBox(height: 6),
          ],
  ...
  Future<void> _save() async {
    setState(() => _saving = true);
    try {
      await widget.controller.saveSmartProfile({
        ...widget.profile,
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  '${decision['title'] ?? fileName(decision['path'])}',
                  style: const TextStyle(fontWeight: FontWeight.w700),
                ),
                const SizedBox(height: 4),
                Text(
                  '${decision['reason'] ?? 'No reason was recorded.'}',
                  style: const TextStyle(
                    color: ByteSqueezeColors.muted,
                    fontSize: 12.5,
                  ),
